...
search_text_changed (GtkEntry *entry)
{
  ...
  GtkTextIter start, match_start, match_end;
    //Взятие текста из виджета Entry
  text = gtk_entry_get_text (entry);
 //Проверка на пустой текст
  if (text[0] == '\0')
    return;
  //Взятие виджета из верхнего уровня иерархии объектов
  ...
  /* Very simple-minded search implementation */
    //Реализация поиска
    //инициализация итератора с первой позиции в текстовом буфере
...
update_words (ExampleAppWindow *win)
{
  ...
  priv = example_app_window_get_instance_private (win);
  //Получает видимый в данный момент дочерний элемент стека или NULL
  tab = gtk_stack_get_visible_child (GTK_STACK (priv->stack));
